...
class Camera(object):
    ...
    def __init__(self, target, window, stage):
        ...
        self.easing_function = general_settings.EASE_IN
        self.ease_time = 0
    ...
    def update(self, dt):
        if self.destination_x or self.destination_y:
            ...
            if self.destination_x:
                if self.easing_function == general_settings.EASE_IN:
                    self.focus_x = int(easing.ease_in(self.takeoff_x, self.destination_x, self.ease_time, self.ease_time_progress))
                else:
                    self.focus_x = int(easing.ease_out(self.takeoff_x, self.destination_x, self.ease_time, self.ease_time_progress))
            if self.destination_y:
                if self.easing_function == general_settings.EASE_IN:
                    self.focus_y = int(easing.ease_in(self.takeoff_y, self.destination_y, self.ease_time, self.ease_time_progress))
                else:
                    self.focus_y = int(easing.ease_out(self.takeoff_y, self.destination_y, self.ease_time, self.ease_time_progress))
    ...
    def focus_on_target(self, time=0.4, easing=general_settings.EASE_IN):
        x = self.target.x + self.target.half_width
        y = self.target.y + self.target.half_height
        ...
        self.fixed = False
        self.move_to(x, y, time, easing)
    
    
    
    def move_to(self, x, y, time=1, easing=general_settings.EASE_IN):
        self.ease_time = time
